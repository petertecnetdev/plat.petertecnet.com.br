  ...
  useEffect(() => {
    async function fetchItem() {
      try {
        ...
        const res = await axios.get(`${apiBaseUrl}/item/${id}`, { headers: { Authorization: `Bearer ${token}` } });
        const item = res.data;
        setForm({
          name: item.name || '',
          type: item.type || '',
        ...
        Swal.fire('Erro', err.response?.data?.error || 'Não foi possível carregar o item.', 'error');
      } finally {
        setLoading(false);
      }
    }
      ...
      <Container className="mt-4">
        ...
        <Form onSubmit={handleSubmit}>
          <Row className="g-3">
            ...
            <Col md={6}><Form.Group controlId="name"><Form.Label>Nome</Form.Label><Form.Control required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}/></Form.Group></Col>
            <Col md={6}><Form.Group controlId="type"><Form.Label>Tipo</Form.Label><Form.Control value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))}/></Form.Group></Col>
            <Col md={6}><Form.Group controlId="description"><Form.Label>Descrição</Form.Label><Form.Control as="textarea" rows={2} value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}/></Form.Group></Col>
            <Col md={4}><Form.Group controlId="category"><Form.Label>Categoria</Form.Label><Form.Control value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))}/></Form.Group></Col>
            ...
            <Col md={4}><Form.Group controlId="availability_end"><Form.Label>até</Form.Label><Form.Control type="datetime-local" value={form.availability_end} onChange={e => setForm(f => ({ ...f, availability_end: e.target.value }))}/></Form.Group></Col>
            <Col md={4}><Form.Group controlId="expiration_date"><Form.Label>Expira em</Form.Label><Form.Control type="date" value={form.expiration_date} onChange={e => setForm(f => ({ ...f, expiration_date: e.target.value }))}/></Form.Group></Col>
            <Col md={6}><Form.Group controlId="tags"><Form.Label>Tags (vírgula)</Form.Label><Form.Control value={form.tags} onChange={e => setForm(f => ({ ...f, tags: e.target.value }))}/></Form.Group></Col>
            <Col md={3}><Form.Group controlId="discount"><Form.Label>Desconto (R$)</Form.Label><Form.Control type="number" step="0.01" value={form.discount} onChange={e => setForm(f => ({ ...f, discount: e.target.value }))}/></Form.Group></Col>
            <Col md={3}><Form.Group controlId="is_featured"><Form.Label>Destaque</Form.Label><Form.Select value={form.is_featured} onChange={e => setForm(f => ({ ...f, is_featured: e.target.value }))}><option value="0">Não</option><option value="1">Sim</option></Form.Select></Form.Group></Col>
            <Col md={12}><Form.Group controlId="notes"><Form.Label>Notas</Form.Label><Form.Control as="textarea" rows={2} value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}/></Form.Group></Col>
          </Row>
