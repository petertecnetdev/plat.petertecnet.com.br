...
export default function EstablishmentCreatePage(){
 ...
 const acquisition=useMemo(()=>captureAcquisitionAttribution(),[]);
 ...
 const onSubmit=async(data)=>{const token=localStorage.getItem("token");if(!token){Swal.fire("Erro","Sua sessão expirou. Entre novamente.","error");return;}const payload=new FormData();payload.append("app_id",String(appId));Object.entries(data).forEach(([key,value])=>{if(value!==undefined&&value!==null&&String(value).trim()!=="")payload.append(key,String(value).trim());});segments.forEach(segment=>payload.append("segments[]",segment));if(logo)payload.append("logo",logo);if(background)payload.append("background",background);try{const {data:response}=await axios.post(`${apiBaseUrl}/establishment`,payload,{headers:{Authorization:`Bearer ${token}`,"Content-Type":"multipart/form-data"}});const establishment=response?.establishment||response?.data?.establishment||null;const slug=establishment?.slug;try{window.PeterTecnetTelemetry?.track?.("plat_establishment_created",{label:data.name,target:"establishment_create",metadata:{onboarding_path:showOptionalDetails?"detailed":"quick",category:data.category,next_step:slug?"first_catalog_item":"establishment_list",...acquisitionTelemetryMetadata(acquisition)}});}catch(_){/* telemetry must not interrupt onboarding */}await Swal.fire({icon:"success",title:"Empresa cadastrada",text:slug?"Agora adicione seu primeiro item para colocar o cardápio no ar e começar a receber pedidos.":response?.message||"Empresa cadastrada com sucesso.",confirmButtonText:slug?"Adicionar primeiro item":"Continuar"});if(slug){navigate(`/item/create/${encodeURIComponent(slug)}`,{replace:true,state:{establishment,onboarding:true,itemType:"product",acquisition}});return;}navigate("/establishment",{replace:true,state:{establishmentCreated:true,acquisition}});}catch(error){const apiErrors=error?.response?.data?.errors||{};if(Object.keys(apiErrors).some(field=>!["name","category"].includes(field)))setShowOptionalDetails(true);Swal.fire("Erro",apiMessage(error,"Não foi possível criar o estabelecimento."),error?.response?.status===422?"warning":"error");}};
