...
const App = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const { data } = await axios.get(`${apiV1BaseUrl}/me`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return <Router><SeoManager/><Routes>
    ...
    <Route path="/my-orders" element={protectedRoute(<MyOrdersPage/>)}/>
    <Route path="/my-orders/:id" element={protectedRoute(<OrderTrackingPage/>)}/>
