...
export default function RevenueConversionFunnel({ establishment, averageTicket = 0 }) {
  ...
  const pixRecoveryPresented = Number(counts.frontend_plat_guest_pix_recovery_presented || 0);
  const pixRecoveryCopied = Number(counts.frontend_plat_guest_pix_recovery_copied || 0);
  const pixRecovered = Number(counts.frontend_plat_guest_pix_recovery_paid || 0);
  const recoveredRevenueEstimate = pixRecovered * confirmedTicket;
