...
export default function PeterAccountGateway({ apiBaseUrl, appSlug, children }) {
    ...
    fetch(`${api}/account/sso/exchange`, {
      ...
      .then(async (response) => {
        ...
        if (!response.ok || !payload?.data?.access_token) {
          throw new Error(messageOf(payload, "Não foi possível concluir o acesso entre aplicativos."));
        }
        return payload.data;
      })
      .then((data) => {
  ...
  useEffect(() => {
    ...
    const close = (event) => {
      ...
      if (event.type === "mousedown" && panelRef.current && !panelRef.current.contains(event.target)) {
        setOpen(false);
      }
    };
  ...
  const openApp = async (application) => {
    ...
    try {
      ...
      if (!response.ok || !code) {
        throw new Error(messageOf(payload, "Não foi possível abrir o aplicativo."));
      }
      if (!isPeterUrl(destination)) {
        throw new Error("O endereço deste aplicativo não está configurado com segurança.");
      }
        ...
        <div className="peter-account-transfer__card">
          ...
          {transfer === "error" ? (
            ...
            <>
              ...
              <span className="peter-account-transfer__spinner" />
            </>
          )}
        </div>
      </div>
    ...
    <>
      ...
      {account && applications.length > 0 && (
        <div className="peter-account-launcher" ref={panelRef}>
          <button
            className="peter-account-launcher__button"
            type="button"
            aria-label="Abrir aplicativos Peter Tecnet"
            aria-expanded={open}
            onClick={() => setOpen((v) => !v)}
          >
            <span className="peter-account-launcher__dots">
          ...
          {open && (
            <div className="peter-account-launcher__panel">
              <div className="peter-account-launcher__account">
                <div className="peter-account-launcher__avatar">
                  {account.avatar ? <img src={account.avatar} alt="" /> : initials}
                </div>
                <div>
                  <strong>{[account.first_name, account.last_name].filter(Boolean).join(" ") || account.user_name}</strong>
              ...
              <div className="peter-account-launcher__grid">
                {applications.map((application) => {
                  ...
                  return (
                    <button
                      key={application.id || application.slug}
                      type="button"
                      disabled={!enabled || Boolean(switching)}
                      className={`peter-account-launcher__app${current ? " is-current" : ""}${!application.has_access ? " is-locked" : ""}`}
                      onClick={() => openApp(application)}
                    >
                      <span className="peter-account-launcher__appmark">
                        {String(application.name || application.slug || "P").slice(0, 1).toUpperCase()}
                      </span>
              ...
              <a className="peter-account-launcher__home" href="https://petertecnet.com.br">
                Ecossistema Peter Tecnet
              </a>
            </div>
          )}
        </div>
